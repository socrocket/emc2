...
json_value* json_alloc(block_allocator *allocator) {
  json_value *value = reinterpret_cast<json_value *>(allocator->malloc(sizeof (json_value)));
...
json_value *json_parser::json_parse(char *source,
  char **error_pos,
  char **error_desc,
  int *error_line,
        ...
        if ((unsigned char)*it < '\x20') {
          ...
          case 'u': {
            ...
            if (codepoint <= 0x7F) {
              ...
              *last++ = static_cast<char>(0xE0 | (codepoint >> 12));
              *last++ = static_cast<char>(0x80 | ((codepoint >> 6) & 0x3F));
              *last = static_cast<char>(0x80 | (codepoint & 0x3F));
            }
          } it += 4;
            break;
...
void json_parser::parse(char *source) {
  ...
  for (std::vector<std::pair<std::string, json_value *> >::iterator it = m_jsonValues.begin();
       it != m_jsonValues.end();
       ++it) {
    // char *pvalue = NULL;
    char value[100];
    ...
    case JSON_STRING:
      ...
      break;
    case JSON_INT:
      snprintf(value, sizeof(value), "%d", it->second->int_value);
