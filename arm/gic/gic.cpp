...
/// @{
/// @file gic.cpp
/// Implementation of generic interrupt controller (GIC).
///
/// @date 2010-2014
...
#include "arm/gic/gic.h"
#include "core/base/verbose.h"

SR_HAS_MODULE(ArmGIC);

/// Constructor
ArmGIC::ArmGIC(ModuleName name,
  int paddr,
  int pmask,
  int ncpu,
  int eirq,
  ...
  cpu_stat("cpu_stat"),
  irq_req("cpu_req"),
  irq_init_socket("irq_init_socket", amba::amba_AHB, amba::amba_LT, false),
  irq_ack(&ArmGIC::acknowledged_irq, "irq_ack"),
  irq_in(&ArmGIC::incomming_irq, "irq_in"),
  ...
  assert(ncpu < 17 && "the ARM GIC can only handle up to 16 CPUs");
  ...
  //irq_out_socket.register_invalidate_direct_mem_ptr((ArmGIC*)this, &ArmGIC::invalidate_direct_mem_ptr);

  // Register tlm non blocking transport backward path
  //irq_out_socket.register_nb_transport_bw(this, &ArmGIC::nb_transport_bw, 0);
  //SC_THREAD(AcceptThread);

  if (m_pow_mon) {
    GC_REGISTER_TYPED_PARAM_CALLBACK(&sta_power, gs::cnf::pre_read, ArmGIC, sta_power_cb);
    GC_REGISTER_TYPED_PARAM_CALLBACK(&int_power, gs::cnf::pre_read, ArmGIC, int_power_cb);
  }
  ...
  srInfo()
    ...
    ("eirq", eirq)
    ("pow_mon", powmon)
    ("Created an ArmGIC with this parameters");
}
...
void ArmGIC::init_registers() {
  ...
  .callback(SR_POST_WRITE, this, &ArmGIC::clear_write);

  r.create_register("mpstat", "Multiprocessor Status Register",
    0x10, 0x00000000, MP_STAT_WMASK)
  .callback(SR_PRE_READ, this, &ArmGIC::mpstat_read)
  .callback(SR_POST_WRITE, this, &ArmGIC::mpstat_write);

  r.create_register("broadcast", "Interrupt broadcast Register",
    0x14, 0x00000000, BROADCAST_BM);

  // 3 IRQ(A)MP Registers pinned to 0 to indecate that we are a single GIC
  ...
  for (int i = 0; i < g_ncpu; ++i) {
    r.create_register(gen_unique_name("mask", false),
      ...
      0x40 + 0x4 * i,
      0xFFFFFFFE,
      PROC_MASK_EIM | ArmGIC::PROC_MASK_IM)
    ...
    .callback(SR_POST_WRITE, this, &ArmGIC::force_write);
...
void ArmGIC::start_of_simulation() {
...
void ArmGIC::end_of_elaboration() {
}

// Print execution statistic at end of simulation
void ArmGIC::end_of_simulation() {
  v::report << name() << " ********************************************" << v::endl;
  v::report << name() << " * ARM GIC statistic:" << v::endl;
  v::report << name() << " * ================" << v::endl;
  uint64_t sum = 0;
  for (int i = 1; i < 32; i++) {
...
gs::cnf::callback_return_type ArmGIC::sta_power_cb(
    gs::gs_param_base &changed_param,  // NOLINT(runtime/references)
    gs::cnf::callback_type reason) {
  // Nothing to do !!
  // Static power of ArmGIC is constant !!
  return GC_RETURN_OK;
}

// Internal power callback
gs::cnf::callback_return_type ArmGIC::int_power_cb(
    gs::gs_param_base &changed_param,  // NOLINT(runtime/references)
    gs::cnf::callback_type reason) {
  // Nothing to do !!
  // Internal power of ArmGIC is constant !!
  return GC_RETURN_OK;
}
...
void ArmGIC::incomming_irq(const std::pair<uint32_t, bool> &irq, const sc_time &time) {
  if (!irq.second) {
...
void ArmGIC::acknowledged_irq(const uint32_t &irq, const uint32_t &cpu, const sc_core::sc_time &time) {
  bool f = false;
