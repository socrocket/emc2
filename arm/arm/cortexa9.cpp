...
/// @addtogroup leon3_CortexA9
/// @{
/// @file leon3_CortexA9.cpp
/// Implementation of LEON2/3 cache-subsystem consisting of instruction cache,
/// data cache, i/d localrams and memory management unit. The leon3_CortexA9 class
...
CortexA9::CortexA9(
  ...
  g_stdout_filename("stdout_filename", "", m_generics) {
    ...
    GC_REGISTER_TYPED_PARAM_CALLBACK(&g_gdb, gs::cnf::post_write, CortexA9, g_gdb_callback);
    GC_REGISTER_TYPED_PARAM_CALLBACK(&g_args, gs::cnf::post_write, CortexA9, g_args_callback);
    CortexA9::init_generics();
    cpu.tool_manager.add_tool(m_intrinsics);
}
...
void CortexA9::start_of_simulation() {
  g_args_callback(g_args, gs::cnf::no_callback);
}

void CortexA9::clkcng() {
  mmu_cache_base::clkcng();
  // TODO: ARM Clock
  //cpu.latency = clock_cycle;
}

gs::cnf::callback_return_type CortexA9::g_gdb_callback(gs::gs_param_base& changed_param, gs::cnf::callback_type reason) {
  int port = 0;
  changed_param.getValue(port);
  if(port) {
    debugger = new GDBStub<uint32_t>(cpu.ABIIf);
    cpu.tool_manager.add_tool(*debugger);
...
gs::cnf::callback_return_type CortexA9::g_args_callback(gs::gs_param_base& changed_param, gs::cnf::callback_type reason) {
...
// Read dword
sc_dt::uint64 CortexA9::read_dword(
...
void CortexA9::unlock() {

}

void CortexA9::trigger_exception(unsigned int exception) {
