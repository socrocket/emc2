...
/// @addtogroup leon3_MicroBlaze
/// @{
/// @file leon3_MicroBlaze.cpp
/// Implementation of LEON2/3 cache-subsystem consisting of instruction cache,
/// data cache, i/d localrams and memory management unit. The leon3_MicroBlaze class
...
MicroBlaze::MicroBlaze(
  ...
  g_stdout_filename("stdout_filename", "", m_generics) {
    ...
    GC_REGISTER_TYPED_PARAM_CALLBACK(&g_gdb, gs::cnf::post_write, MicroBlaze, g_gdb_callback);
    GC_REGISTER_TYPED_PARAM_CALLBACK(&g_args, gs::cnf::post_write, MicroBlaze, g_args_callback);
    MicroBlaze::init_generics();
...
void MicroBlaze::start_of_simulation() {
  g_args_callback(g_args, gs::cnf::no_callback);
}

void MicroBlaze::clkcng() {
  mmu_cache_base::clkcng();
  cpu.latency = clock_cycle;
}

gs::cnf::callback_return_type MicroBlaze::g_gdb_callback(gs::gs_param_base& changed_param, gs::cnf::callback_type reason) {
  int port = 0;
...
gs::cnf::callback_return_type MicroBlaze::g_args_callback(gs::gs_param_base& changed_param, gs::cnf::callback_type reason) {
...
// Read instruction
unsigned int MicroBlaze::read_instr(const unsigned int & address, const unsigned int asi, const unsigned int flush) throw() {
...
// Read dword
sc_dt::uint64 MicroBlaze::read_dword(
...
sc_dt::uint64 MicroBlaze::read_dword_dbg(const uint32_t &address) throw() {
...
uint32_t MicroBlaze::read_word_dbg(const uint32_t &address) throw(){
    uint32_t debug = 0;
    sc_time delay = this->cpu.quantKeeper.get_local_time();
    uint32_t datum = 0;
    tlm::tlm_response_status response = tlm::TLM_INCOMPLETE_RESPONSE;
...
uint16_t MicroBlaze::read_half_dbg(const uint32_t &address) throw() {
    uint32_t debug = 0;
    sc_time delay = this->cpu.quantKeeper.get_local_time();
    uint16_t datum = 0;
    tlm::tlm_response_status response = tlm::TLM_INCOMPLETE_RESPONSE;
...
uint8_t MicroBlaze::read_byte_dbg(const uint32_t &address) throw(){
    uint32_t debug = 0;
    sc_time delay = this->cpu.quantKeeper.get_local_time();
    uint8_t datum = 0;
    tlm::tlm_response_status response = tlm::TLM_INCOMPLETE_RESPONSE;
...
void MicroBlaze::write_dword_dbg(const uint32_t &address, sc_dt::uint64 datum) throw() {
...
void MicroBlaze::write_word_dbg(const uint32_t &address, uint32_t datum) throw() {
    //Now the code for endianess conversion: the processor is always modeled
    //with the host endianess; in case they are different, the endianess
    //is turned
    swapEndianess(datum);
...
void MicroBlaze::write_half_dbg(const uint32_t &address, uint16_t datum) throw() {
    //Now the code for endianess conversion: the processor is always modeled
    //with the host endianess; in case they are different, the endianess
    //is turned
    swapEndianess(datum);
...
void MicroBlaze::write_byte_dbg(const uint32_t &address, uint8_t datum) throw() {
    uint32_t debug = 0;
    sc_time delay = this->cpu.quantKeeper.get_local_time();
    tlm::tlm_response_status response = tlm::TLM_INCOMPLETE_RESPONSE;
...
void MicroBlaze::lock() {

}

void MicroBlaze::unlock() {

}

void MicroBlaze::trigger_exception(unsigned int exception) {
